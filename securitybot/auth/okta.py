...
class OktaAuth(Auth):
    ...
    def __init__(self, okta_api_key, okta_api_endpoint, domain_name, username):
        # type: (Any, str) -> None
        '''
        Args:
            okta_api_key (str): The api token generated by Okta.
            okta_api_endpoint (str): The full okta URL (https://myorg.okta.com).
            domain_name (str): The domain name to search against for users.
                               For example, if your org's emails go to
                               <user>@myorg.com, this would be 'myorg.com'.
    ...
    def _get_okta_userid(self, username):
        searchFilter = 'profile.login eq "%s@%s"' % (username, self.domain)
        r = self._make_get_request('users', queryParameters={'filter': searchFilter})
        try:
            return r[0]['id']
        except:
            return None
